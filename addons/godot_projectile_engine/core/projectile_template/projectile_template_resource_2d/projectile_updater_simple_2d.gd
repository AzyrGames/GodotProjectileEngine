...
var projectile_speed : float = 100:
	set(value):
		...
		# todo Reupdate velocity for every instance
...
var _active_instances : Array[ProjectileInstanceSimple2D]
var _projectile_instance : ProjectileInstanceSimple2D
...
func create_projectile_pool() -> void:
	...
	for _index in range(projectile_max_pooling):
		...
		_projectile_instance = ProjectileInstanceSimple2D.new()
...
func clear_projectile_updater() -> void:
	for instance : ProjectileInstanceSimple2D in projectile_instance_array:
		instance.queue_free()
